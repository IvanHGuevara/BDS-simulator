...
class EvolvableSelector(AmbulanceSelector):
    ...
    def runGP(self, times):
        ...
        toolbox.register("evaluate", self.evalSymbReg, times)
    ...
    def evalSymbReg(self, individual, ambulanceTime):
        # Transform the tree expression in a callable function
        func = self.toolbox.compile(expr=individual)
        value = func(ambulanceTime)
        return value,
    ...
    def select_ambulance(self,
        ...
        minValue = self.runGP(list(map(lambda timeList: timeList[0], ambulances_times)))
        ambulanceSelected = filter(lambda ambTime: ambTime[0] == minValue, ambulances_times)[0]
